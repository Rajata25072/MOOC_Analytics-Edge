...
# It is available in many languages and is growing all the time. 
# On the English language version of Wikipedia:
#   
...
# or more subtle shifts that change the meaning of the article. 
# With this many articles and edits per day it is difficult for humans to detect all 
# instances of vandalism and revert (undo) them. 
# As a result, Wikipedia uses bots - computer programs that automatically revert 
# edits that look like vandalism. 
# In this assignment we will attempt to develop a vandalism detector that uses 
# machine learning to distinguish between a valid edit and vandalism.
# 
# The data for this problem is based on the revision history of the page Language. 
# Wikipedia provides a history for each page that consists of the state of the 
# page at each revision. 
# Rather than manually considering each revision, a script was run that checked 
# whether edits stayed or were reverted. 
# If a change was eventually reverted then that revision is marked as vandalism. 
# This may result in some misclassifications, but the script performs well enough 
# for our needs.
# As a result of this preprocessing, some common processing tasks have already 
# been done, including lower-casing and punctuation removal. 
# The columns in the dataset are:
#   
#   Vandal = 1 if this edit was vandalism, 0 if not.
#   Minor = 1 if the user marked this edit as a "minor edit", 0 if not.
#   Loggedin = 1 if the user made this edit while using a Wikipedia account, 
#     0 if they did not.
#   Added = The unique words added.
#   Removed = The unique words removed.
# 
# Notice the repeated use of unique. 
# The data we have available is not the traditional bag of words - rather it is the 
# set of words that were removed or added. 
# For example, if a word was removed multiple times in a revision it will only 
# appear one time in the "Removed" column.
# 
# 
# ################################################################################
# Problem 1.1 - Bags of Words
# Load the data wiki.csv with the option stringsAsFactors=FALSE, calling the data 
# frame "wiki". 
...
wiki$Vandal = as.factor(wiki$Vandal)
sum(wiki$Vandal==1) #1815
# 
# 
# ################################################################################
# Problem 1.2 - Bags of Words
# We will now use the bag of words approach to build a model. 
# We have two columns of textual data, with different meanings. 
# For example, adding rude words has a different meaning to removing rude words. 
# We'll start like we did in class by building a document term matrix from the Added column. 
...
# then please run the line of code in this file, which will store the standard stop 
# words in a variable called sw. 
# When removing stop words, use tm_map(corpusAdded, removeWords, sw) 
# instead of tm_map(corpusAdded, removeWords, stopwords("english")).
# 
...
# words, stem the document, create a sparse document term matrix, and 
# convert it to a data frame) to create a Removed bag-of-words dataframe, 
# called wordsRemoved, except this time, prepend all of the words with the 
# letter R:
...
# into one data frame. Then add the Vandal column 
# (HINT: remember how we added the dependent variable back into our data 
# frame in the Twitter lecture). 
# Set the random seed to 123 and then split the data set using sample.split from 
# the "caTools" package to put 70% in the training set.
...
sum(wikiTest$Vandal==0)/nrow(wikiTest) #0.5313844
# 
# 
# ################################################################################
...
# (Remember that if you add the argument type="class" when making 
# predictions, the output of predict will automatically use a threshold of 0.5.)
library(rpart)
library(rpart.plot)
...
#  
# x We have a bad testing/training split.  
# x The CART model overfits to the training set.  
# o Although it beats the baseline, bag of words is not very predictive for this problem.  
# x We over-sparsified the document-term matrix.
# 
# 
# EXPLANATION
# 
...
#   on the training set. 
# Over-sparsification is plausible but unlikely, since we selected a very high 
#   sparsity parameter. 
...
# There are other options though, and in this section we will try two techniques - 
# identifying a key class of words, and counting words.
# 
# The key class of words we will use are website addresses. 
# "Website addresses" (also known as URLs - Uniform Resource Locators) are 
# comprised of two main parts. 
# An example would be "http://www.google.com". The first part is the protocol, 
# which is usually "http" (HyperText Transfer Protocol). 
# The second part is the address of the site, e.g. "www.google.com". 
# We have stripped all punctuation so links to websites appear in the data as one 
# word, e.g. "httpwwwgooglecom". 
# We hypothesize that given that a lot of vandalism seems to be adding links to 
# promotional or irrelevant websites, the presence of a web address is a sign of 
# vandalism.
# 
# We can search for the presence of a web address in the words added by 
# searching for "http" in the Added column. 
...
# observations to put in the training and testing sets. 
# Use that variable (do not recompute it with sample.split) to make new training 
# and testing sets:
...
# of 0.5?
predictCART = predict(wikiCART, newdata=wikiTest2, type="class")
sum(table(wikiTest2$Vandal, predictCART) * diag(x=1,2,2)) / nrow(wikiTest2) #0.5726569
# 
# 
# ################################################################################
...
# We already have a word count available in the form of the document-term 
# matrices (DTMs).
# 
...
sum(table(wikiTest3$Vandal, predictCART3) * diag(x=1,2,2)) / nrow(wikiTest3) #0.6552021
# 
# 
# ################################################################################
...
sum(table(wikiTest4$Vandal, predictCART4) * diag(x=1,2,2)) / nrow(wikiTest4) #0.7188306
# 
# 
# ################################################################################
# Problem 3.2 - Using Non-Textual Data
# There is a substantial difference in the accuracy of the model using the 
# meta data. 
