...
# In this script file, we show how the airline revenue management problems discussed in this lecture can be
# solved in R using the "lpSolveAPI" package. 
# This package allows you to solve linear optimization problems in R using the lp_solve library, 
# which is an open-sourced optimization solver. 

# If you are solving larger and more complex optimization problems, lp_solve might not be powerful enough to 
# solve your problems. 
# There are other optimization packages available in R that rely on commercial solvers, such as cplexAPI and Rcplex.
# For more information, see the CRAN Optimization page: http://cran.us.r-project.org/web/views/Optimization.html
...
# Now let's start by solving the simple airline problem with just two decision variables: 
# the number of regular seats to offer, and the number of discount seats to offer. 
...
# Now we need to set up the constraints and objective. The best way to do this using the lpSolveAPI package is 
# by viewing the constraints in a matrix format. Our objective and three constraints are as follows:
# max         617*R + 238*D
...
# You should see that there is a row called "Lower" in the model output - the values of zero indicate that
# our decision variables are constrained to be non-negative, which is the default setting. If you want to 
# change the lower or upper bounds for a decision variable, you can do so with the set.bounds function. 
# (Next week we will learn about integer and binary decision variables - to change a decision variable to
#  one of these types, you can use the set.type function.)

# Now we are ready to solve our model:

solve(AirlineSimple)

# An output of zero means that the model was successfully solved. You can look at the optimal objective function
# value or optimal decision variable values with get.objective and get.variables:

get.objective(AirlineSimple)
get.variables(AirlineSimple)

# You should get an objective value of 77408 and decision variable values of 100 and 66, just like we did in
# lecture.
