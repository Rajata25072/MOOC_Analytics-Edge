...
# According to the National Vital Statistics Reports published in October 2012, influenza ranked 
# as the eighth leading cause of death in 2011 in the United States. 
# Each year, 250,000 to 500,000 deaths are attributed to influenza related diseases throughout 
# the world.
# 
# The U.S. Centers for Disease Control and Prevention (CDC) and the European Influenza Surveillance 
# Scheme (EISS) detect influenza activity through virologic and clinical data, including 
# Influenza-like Illness (ILI) physician visits. 
...
#     by dividing the count for each query in a particular week by the total number of 
#     online search queries submitted in that location during the week. Then, the values are 
...
# which week corresponds to the highest percentage of ILI-related physician visits? Select the day 
# of the month corresponding to the start of this week.
FluTrain = read.csv("FluTrain.csv") 
summary(FluTrain)
head(FluTrain); tail(FluTrain)
FluTrain[which.max(FluTrain$ILI),] #2009-10-18 - 2009-10-24
# Which week corresponds to the highest percentage of ILI-related query fraction?
FluTrain[which.max(FluTrain$Queries),] #2009-10-18 - 2009-10-24
# 
# ##################################################################################
...
# the dependent variable instead of the dependent variable itself -- this prevents the small 
# number of unusually large or small observations from having an undue influence on the sum 
# of squared errors of predictive models. 
# In this problem, we will predict the natural log of the ILI variable, which can be computed 
# in R using the log() function.
# 
...
# choice. 
# Based on our understanding of the data from the previous subproblem, which model best describes
# our estimation problem?
# 
#   xILI = intercept + coefficient x Queries, where the coefficient is negative  
#   xQueries = intercept + coefficient x ILI, where the coefficient is negative  
#   xILI = intercept + coefficient x Queries, where the coefficient is positive  
#   xQueries = intercept + coefficient x ILI, where the coefficient is positive  
#   xlog(ILI) = intercept + coefficient x Queries, where the coefficient is negative  
#   xQueries = intercept + coefficient x log(ILI), where the coefficient is negative  
#   olog(ILI) = intercept + coefficient x Queries, where the coefficient is positive  
#   xQueries = intercept + coefficient x log(ILI), where the coeffcient is positive
# 
# 
# ##################################################################################
...
# What is the relationship we infer from our problem? (Don't forget that you can use the 
# cor function to compute the correlation between two variables.)
cor(log(FluTrain$ILI), FluTrain$Queries)^2
# oR-squared = Correlation^2  
# xR-squared = log(1/Correlation)  
# xR-squared = exp(-0.5*Correlation)
# 
# Note that the "exp" function stands for the exponential function. The exponential can be
# computed in R using the function exp().
# 
# 
# ##################################################################################
...
# the observed weekly percentage of ILI-related physician visits. 
...
# predictions of the log(ILI) value. 
...
# March 11, 2012? 
# (HINT: You can either just output FluTest$Week to find which element corresponds to 
#  March 11, 2012, or you can use the "which" function in R. To learn more about the 
...
pos=which(FluTest$Week=="2012-03-11 - 2012-03-17")
(FluTest$ILI[pos] - PredTest1[pos]) / FluTest$ILI[pos] #0.04623827
# 
# ##################################################################################
...
SSE_test = sum((FluTest$ILI - PredTest1)^2)
RMSE_test = sqrt(SSE_test / length(PredTest1)); RMSE_test #0.7490645
# 
# ##################################################################################
...
# This sort of dataset is called a "time series." Often, statistical models can be improved by 
# predicting the current value of the dependent variable using the value of the dependent variable 
# from earlier weeks. 
# In our models, this means we will predict the ILI variable in the current week using values of 
# the ILI variable from previous weeks.
# 
# First, we need to decide the amount of time to lag the observations. 
# Because the ILI variable is reported with a 1- or 2-week lag, a decision maker cannot rely on 
# the previous week's ILI value to predict the current week's value. 
...
# New packages can be installed and loaded easily in R, and we will do this many times in this class. 
# Run the following two commands to install and load the zoo package. 
...
# the current one; a positive value would have returned future observations. 
...
# Train a linear regression model on the FluTrain dataset to predict the log of the ILI variable
# using the Queries variable as well as the log of the ILILag2 variable. Call this model FluTrend2.
FluTrend2 = lm(log(ILI) ~ Queries + log(ILILag2), data=FluTrain)
# Which coefficients are significant at the p=0.05 level in this regression model? 
# (Select all that apply.)
summary(FluTrend2)
#   oIntercept  oQueries  olog(ILILag2)
# 
# 
...
# On the basis of R-squared value and significance of coefficients, which statement is 
# the most accurate?
summary(FluTrend1) 
summary(FluTrend2) 
#   xDue to overfitting, FluTrend2 is a weaker model then FluTrend1 on the training set.  
#   xFluTrend2 is about the same quality as FluTrend1 on the training set.  
#   oFluTrend2 is a stronger model than FluTrend1 on the training set.
# 
# 
# ##################################################################################
...
# To make predictions with our FluTrend2 model, we will also need to add ILILag2 to the
# FluTest data frame (note that adding variables before splitting into a training and testing set
...
# In this problem, the training and testing sets are split sequentially -- the training set contains
# all observations from 2004-2011 and the testing set contains all observations from 2012. 
# There is no time gap between the two datasets, meaning the first observation in FluTest 
# was recorded one week after the last observation in FluTrain. 
...
# 
#   oThe ILI value of the second-to-last observation in the FluTrain data frame.  
#   xThe ILI value of the last observation in the FluTrain data frame.  
#   xThe ILI value of the first observation in the FluTest data frame.  
#   xThe ILI value of the second observation in the FluTest data frame.
# 
# Which value should be used to fill in the ILILag2 variable for the second observation in FluTest?
# 
#   xThe ILI value of the second-to-last observation in the FluTrain data frame.  
#   oThe ILI value of the last observation in the FluTrain data frame. 
#   xThe ILI value of the first observation in the FluTest data frame.  
#   xThe ILI value of the second observation in the FluTest data frame.
# 
# 
...
# In terms of syntax, you could set the value of ILILag2 in row "x" of the FluTest data frame 
# to the value of ILI in row "y" of the FluTrain data frame with 
# "FluTest$ILILag2[x] = FluTrain$ILI[y]". 
# Use the answer to the previous questions to determine the appropriate values of "x" and "y". 
# It may be helpful to check the total number of rows in FluTrain using str(FluTrain) or 
# nrow(FluTrain).
nrow(FluTrain) #417 row
...
# Obtain test set predictions of the ILI variable from the FluTrend2 model, again remembering to
# call the exp() function on the result of the predict() function to obtain predictions 
# for ILI instead of log(ILI).
Flupredict2 = exp(predict(FluTrend2, newdata=FluTest))
# 
# What is the test-set RMSE of the FluTrend2 model?
SSE_test2 =  sum((Flupredict2 - FluTest$ILI)^2)
RMSE_test2 = sqrt(SSE_test2/nrow(FluTest)); RMSE_test2 #0.2942029
# 
# ##################################################################################
# Problem 5.5 - Evaluating the Time Series Model in the Test Set
# Which model obtained the best test-set RMSE?
Flupredict1 = exp(predict(FluTrend1, newdata=FluTest))
SSE_test1 =  sum((Flupredict1 - FluTest$ILI)^2)
RMSE_test1 = sqrt(SSE_test1/nrow(FluTest)); RMSE_test1 #0.7490645 
# FluTrend2
# 
# In this problem, we used a simple time series model with a single lag term. 
# ARIMA models are a more general form of the model we built, which can include multiple lag terms 
# as well as more complicated combinations of previous values of the dependent variable. 
# If you're interested in learning more, check out ?arima or the available online tutorials 
