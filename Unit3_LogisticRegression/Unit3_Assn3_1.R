...
# 
# Artists are at the core of the music industry and record labels provide them with the
# necessary resources to sell their music on a large scale. 
# A record label incurs numerous costs (studio recording, marketing, distribution, and touring) 
# in exchange for a percentage of the profits from album sales, singles and concert tickets.
# 
# Unfortunately, the success of an artist's release is highly uncertain: a single may be 
# extremely popular, resulting in widespread radio play and digital downloads, while another single
# may turn out quite unpopular, and therefore unprofitable. 
# 
# Knowing the competitive nature of the recording industry, record labels face the fundamental 
# decision problem of which musical releases to support to maximize their financial success. 
# 
# How can we use analytics to predict the popularity of a song? 
# In this assignment, we challenge ourselves to predict whether a song will reach a spot in the 
# Top 10 of the Billboard Hot 100 Chart.
# 
# Taking an analytics approach, we aim to use information about a song's properties to predict 
# its popularity. 
# The dataset songs.csv consists of all songs which made it to the Top 10 of the Billboard 
# Hot 100 Chart from 1990-2010 plus a sample of additional songs that didn't make the Top 10. 
# This data comes from three sources: Wikipedia, Billboard.com, and EchoNest.
# 
# The variables included in the dataset either describe the artist or the song, or they are 
# associated with the following song attributes: time signature, loudness, key, pitch, tempo, 
# and timbre.
# 
# Here's a detailed description of the variables:
#   
#   year = the year the song was released
#   v = the title of the song
#   artistname = the name of the artist of the song
#   songID and artistID = identifying variables for the song and artist
#   timesignature and timesignature_confidence = a variable estimating the time signature of 
#     the song, and the confidence in the estimate
...
#   energy = a variable that represents the overall acoustic energy of the song, using a mix of 
#     features such as loudness
#   pitch = a continuous variable that indicates the pitch of the song
#   timbre_0_min, timbre_0_max, timbre_1_min, timbre_1_max, . . . , timbre_11_min, and timbre_11_max =
#     variables that indicate the minimum/maximum values over all segments for each of the twelve 
#     values in the timbre vector (resulting in 24 continuous variables)
#   Top10 = a binary variable indicating whether or not the song made it to the Top 10 of 
#       the Billboard Hot 100 Chart (1 if it was in the top 10, and 0 if it was not)
# 
# ##################################################################################    
...
MichaelJackson[c("songtitle", "Top10")]
MichaelJackson[c("songtitle", "Top10")][MichaelJackson$Top10 == 1,]
# xBeat It
# oYou Rock My World
# xBillie Jean
# oYou Are Not Alone
...
songs[c("songtitle","tempo")][which.max(songs$tempo),]
...
# To do this, first use the subset function to split the data into a training set 
# "SongsTrain" consisting of all the observations up to and including 2009 song releases, 
# and a testing set "SongsTest", consisting of the 2010 song releases.
# 
# How many observations (songs) are in the training set?
SongsTrain = subset(songs, songs$year <= 2009)
SongsTest = subset(songs, songs$year == 2010)
nrow(SongsTrain) #7201
# 
# ##################################################################################    
# Problem 2.2 - Creating our Prediction Model
# In this problem, our outcome variable is "Top10" - we are trying to predict whether or not 
# a song will make it to the Top 10 of the Billboard Hot 100 Chart. 
...
# 
# We will only use the variables in our dataset that describe the numerical attributes of the song 
...
# explicitly input the formula including all the independent variables in R. 
# However, in this case, this is a tedious amount of work since we have a large number of 
# independent variables.
# 
# There is a nice trick to avoid doing so. Let's suppose that, except for the outcome variable 
# Top10, all other variables in the training set are inputs to Model 1. 
...
# variables. 
# (Also, keep in mind that you can choose to put quotes around binomial, or leave out the quotes. 
# R can understand this argument either way.)
# 
# However, in our case, we want to exclude some of the variables in our dataset from being used 
# as independent variables ("year", "songtitle", "artistname", "songID", and "artistID"). 
# To do this, we can use the following trick. 
# First define a vector of variable names called nonvars - these are the variables that 
# we won't use in our model.
# 
   nonvars = c("year", "songtitle", "artistname", "songID", "artistID")
# 
# To remove these variables from your training and testing sets, type the following commands in 
# your R console:
# 
...
summary(SongsLog1) #4827.2
# 
# ##################################################################################    
# Problem 2.3 - Creating Our Prediction Model
# Let's now think about the variables in our dataset related to the confidence of the time signature, 
...
# 
# The higher our confidence about time signature, key and tempo, the more likely the song is to be 
# in the Top 10
# 
# 
# ##################################################################################    
# Problem 2.4 - Creating Our Prediction Model
# In general, if the confidence is low for the time signature, tempo, and key, then the song is 
...
# In the rest of this problem, we'll build two variations of our original model: 
#   Model 2, in which we keep "energy" and omit "loudness", 
#   and Model 3, in which we keep "loudness" and omit "energy".
# 
# 
# ##################################################################################    
...
# and "artistname", because they are not numeric variables, and we might get different values in 
# the test set that the training set has never seen. But this approach (subtracting the variable 
# from the model formula) will always work when you want to remove numeric variables.
# 
# Look at the summary of SongsLog2, and inspect the coefficient of the variable "energy". 
# What do you observe?
summary(SongsLog2)
# 
...
# do we make the same observation about the popularity of heavy instrumentation as we did 
# with Model 2?
summary(SongsLog3)
# 
# 
...
# What would the accuracy of the baseline model be on the test set? (Give your answer as 
...
# in investing in songs that are highly likely to make it to the Top 10. 
# The company's objective is to minimize its risk of financial losses attributed to investing 
# in songs that end up unpopular.
# 
# 
# A competitive edge can therefore be achieved if we can provide the production company 
# a list of songs that are highly likely to end up in the Top 10. 
# We note that the baseline model does not prove useful, as it simply does not label any song as 
# a hit. Let us see what our model has to offer.
# 
# How many songs does Model 3 correctly predict as Top 10 hits in 2010 
# (remember that all songs in 2010 went into our test set), using a threshold of 0.45?
table(SongsTest$Top10, predictTest > 0.45) #19
# 
# 
# 
# How many non-hit songs does Model 3 predict will be Top 10 hits (again, looking at the test set), 
# using a threshold of 0.45?
table(SongsTest$Top10, predictTest > 0.45) #5
# 
# ##################################################################################    
...
# 
# oModel 3 favors specificity over sensitivity.  
# xModel 3 favors sensitivity over specificity.  
# xModel 3 captures less than half of Top 10 songs in 2010. Model 3 therefore does not 
#   provide a useful list of candidate songs to investors, and hence offers no competitive edge.  
# oModel 3 provides conservative predictions, and predicts that a song will make it to the 
#   Top 10 very rarely. So while it detects less than half of the Top 10 songs, we can be 
#   very confident in the songs that it does predict to be Top 10 hits.
